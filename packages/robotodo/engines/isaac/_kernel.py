...
class Kernel:
    ...
    def __init__(
        ...
        with _undo_omni_monkeypatching():
            ...
            # kit_app.startup("kit", os.environ["CARB_APP_PATH"], argv)
            kit_app = AppFramework(argv=argv).app

            # TODO
            while not kit_app.is_app_ready():
                kit_app.update()

            self._kit_app = kit_app
    ...
    def shutdown_app(self):
        self._kit_app.shutdown()

    @property
    def app(self):
        return self._kit_app
    ...
    def enable_extension(
        ...
        ext_manager = self._kit_app.get_extension_manager()
        if ext_manager.is_extension_enabled(name) is enabled:
            return
        if immediate:
            ext_manager.set_extension_enabled_immediate(name, enabled)
